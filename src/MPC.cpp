 ...
 public:
  ...
  void operator()(ADvector& fg, const ADvector& vars) {
    ...
    for (int t = 1; t < N; t++) {
      ...
      AD<double> f0_dash = coeffs[1] + coeffs[2] * 2 * x0 + coeffs[3] * 3 * x0 * x0;
      AD<double> psides0 = CppAD::atan(f0_dash);
...
vector<double> MPC::Solve(Eigen::VectorXd state, Eigen::VectorXd coeffs) {
  ...
  for (int i=0; i<N; i++) {
    mpc_out.push_back(solution.x[x_start + i]);
    mpc_out.push_back(solution.x[y_start + i]);
  }
  return mpc_out;
}
